...
async function search(){
  ...
  global.serach.type = urlPrams.get('type');
  global.serach.term = urlPrams.get('search-term');

  if(global.serach.term !== '' && global.serach.term !==null){
...
function displaySearchResults(results){
  results.forEach((result) => {
    ...
    div.classList.add("card");
    div.innerHTML = `<div>
   <a href="${global.serach.type}-details.html?id=${result.id}">
     ${
       result.poster_path
         ? `<img 
     src="https://image.tmdb.org/t/p/w500${result.poster_path}"
     class="card-img-top"
     alt="${global.serach.type === 'movie' ? result.title : result.name}"
   />`
         : `<img
   src="../images/no-image.jpg"
   class="card-img-top"
  ...
  />`
   ...
   <div class="card-body">
     <h5 class="card-title">${global.serach.type === 'movie' ? result.title : result.name}</h5>
     <p class="card-text">
       <small class="text-muted">Release: ${global.serach.type === 'movie' ? result.release_date : result.first_air_date}</small>
     </p>
   </div>
   </div>`;
...
async function serachApiData(){
  const API_KEY =  global.api.apiKey;
  const API_URL = global.api.apiUrl;
  const searchTerm = global.serach.term;

  showSpinner();

  const resp = await fetch(
    `${API_URL}search/${global.serach.type}?api_key=${API_KEY}&language=en-US&query=${searchTerm}`
  );
  const data = await resp.json();
  hideSpinner();
  return data;
